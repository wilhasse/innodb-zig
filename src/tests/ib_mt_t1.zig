...
fn create_t1(arg: *mt.CbArgs) api.ib_err_t {
    ...
    err = api.ib_table_schema_add_col(tbl_sch, "first", .IB_VARCHAR, .IB_COL_NONE, 0, 128);
    if (err != .DB_SUCCESS) {
        mt.updateErrStats(arg.err_st, err);
        return err;
    }
    err = api.ib_table_schema_add_col(tbl_sch, "last", .IB_VARCHAR, .IB_COL_NONE, 0, 128);
    if (err != .DB_SUCCESS) {
        mt.updateErrStats(arg.err_st, err);
        return err;
    }
    ...
    err = api.ib_table_schema_add_index(tbl_sch, "first_last", &idx_sch);
    if (err != .DB_SUCCESS) {
        mt.updateErrStats(arg.err_st, err);
        return err;
    }
    err = api.ib_index_schema_add_col(idx_sch, "first", 0);
    if (err != .DB_SUCCESS) {
        mt.updateErrStats(arg.err_st, err);
        return err;
    }
    err = api.ib_index_schema_add_col(idx_sch, "last", 0);
    if (err != .DB_SUCCESS) {
        mt.updateErrStats(arg.err_st, err);
        return err;
    }
...
fn alter_t1(_: *mt.CbArgs) api.ib_err_t {
    return .DB_SUCCESS;
}

fn randChar(prng: *std.Random.DefaultPrng) u8 {
    return @as(u8, 'a') + @as(u8, @intCast(prng.random().intRangeAtMost(u8, 0, 25)));
}

fn insert_t1(arg: *mt.CbArgs) api.ib_err_t {
    var prng = std.Random.DefaultPrng.init(@intCast(arg.run_number));
    ...
    var i: usize = 0;
    while (i < batch) : (i += 1) {
        var first_buf: [2]u8 = .{ randChar(&prng), randChar(&prng) };
        var last_buf: [2]u8 = .{ randChar(&prng), randChar(&prng) };

        err = api.ib_col_set_value(tpl, 0, &first_buf, 2);
        if (err != .DB_SUCCESS) break;
        err = api.ib_col_set_value(tpl, 1, &last_buf, 2);
        if (err != .DB_SUCCESS) break;
        err = api.ib_tuple_write_u32(tpl, 2, 0);
        if (err != .DB_SUCCESS) break;
        err = api.ib_tuple_write_u32(tpl, 3, @intCast(arg.run_number));
        if (err != .DB_SUCCESS) break;
        err = api.ib_tuple_write_u32(tpl, 4, 0);
        if (err != .DB_SUCCESS) break;
...
fn update_t1(arg: *mt.CbArgs) api.ib_err_t {
    var crsr: api.ib_crsr_t = null;
    var err = mt.openTable(mt.sliceName(&arg.tbl.db_name), mt.sliceName(&arg.tbl.name), arg.trx, &crsr);
    if (err != .DB_SUCCESS) {
        mt.updateErrStats(arg.err_st, err);
    ...
    err = api.ib_col_set_value(key_tpl, 0, "a", 1);
    if (err != .DB_SUCCESS) {
        mt.updateErrStats(arg.err_st, err);
        return err;
    }
    ...
    err = api.ib_tuple_read_u32(old_tpl, 2, &score);
    if (err != .DB_SUCCESS) {
        mt.updateErrStats(arg.err_st, err);
        return err;
    }
    score += 100;
    err = api.ib_tuple_write_u32(new_tpl, 2, score);
    if (err != .DB_SUCCESS) {
        mt.updateErrStats(arg.err_st, err);
        return err;
    }
    err = api.ib_tuple_write_u32(new_tpl, 4, @intCast(arg.run_number));
    if (err != .DB_SUCCESS) {
        mt.updateErrStats(arg.err_st, err);
        return err;
    }
...
fn delete_t1(arg: *mt.CbArgs) api.ib_err_t {
    var crsr: api.ib_crsr_t = null;
    var err = mt.openTable(mt.sliceName(&arg.tbl.db_name), mt.sliceName(&arg.tbl.name), arg.trx, &crsr);
    if (err != .DB_SUCCESS) {
        mt.updateErrStats(arg.err_st, err);
    ...
    err = api.ib_col_set_value(key_tpl, 0, "x", 1);
    if (err != .DB_SUCCESS) {
        mt.updateErrStats(arg.err_st, err);
        return err;
    }
    err = api.ib_col_set_value(key_tpl, 1, "z", 1);
    if (err != .DB_SUCCESS) {
        mt.updateErrStats(arg.err_st, err);
        return err;
    }
...
pub fn register_t1_table(tbl: *mt.TblClass) void {
    mt_base.registerBaseTable(tbl);
    mt.setName(&tbl.name, "t1");

    tbl.dml_fn[@intFromEnum(mt.DmlOpType.insert)] = insert_t1;
    tbl.dml_fn[@intFromEnum(mt.DmlOpType.update)] = update_t1;
    tbl.dml_fn[@intFromEnum(mt.DmlOpType.delete)] = delete_t1;

    tbl.ddl_fn[@intFromEnum(mt.DdlOpType.create)] = create_t1;
    tbl.ddl_fn[@intFromEnum(mt.DdlOpType.alter)] = alter_t1;
}

test "ib mt t1 harness" {
